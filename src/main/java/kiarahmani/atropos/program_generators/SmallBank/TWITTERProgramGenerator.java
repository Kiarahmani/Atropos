	...
	public Program generate(String... args) {
		...
		if (txns.contains("insertTweet")) {
			...
			Insert_Query insert1 = pu.addInsertQuery(txn_name, table_name,
					new WHC_Constraint(pu.getTableName(table_name), pu.getFieldName("t_id"), BinOp.EQ, new_id),
					new WHC_Constraint(pu.getTableName(table_name), pu.getFieldName("t_u_id"), BinOp.EQ,
							pu.getArg("it_uid")),
					new WHC_Constraint(pu.getTableName(table_name), pu.getFieldName("t_text"), BinOp.EQ,
							pu.getArg("it_text")),
					new WHC_Constraint(pu.getTableName(table_name), pu.getFieldName("t_createdate"), BinOp.EQ,
							pu.getArg("it_time")));
